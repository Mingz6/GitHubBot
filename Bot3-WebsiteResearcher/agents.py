...
class QuestionAnswerer:
    ...
    def answer_question(self, question, content, source_urls):
        ...
        try:
            ...
            # Stricter content length limit - aim for roughly 10k tokens max
            max_chars = 20000  # Conservative limit since chars != tokens
            if len(content) > max_chars:
                print(f"Truncating content from {len(content)} to {max_chars} characters")
                content = content[:max_chars] + "... [content truncated]"
            
            # Format sources text with limited URLs
            sources_text = "\n".join([f"- {url}" for url in source_urls[:2]])  # Limit to 2 sources
            
            messages = [
                {"role": "system", "content": "You are a web research assistant. Answer the user's question based on the provided website content. Be concise and accurate."},
            ...
            return together_api.chat_completion(
                ...
                messages=messages,
                temperature=0.7,
                max_tokens=512  # Reduced max tokens for response
            )
        except Exception as e:
